...
class ThreadnestRestApiController
{

    /**
     * @var ThreadnestRestApiController
     */
    ...
    {
        if (!isset(self::$instance) && !(self::$instance instanceof ThreadnestRestApiController)) {
            self::$instance = new ThreadnestRestApiController();
        }
    ...
    {
        // Routes are now handled by ThreadnestRestApiRoutes
        ThreadnestRestApiRoutes::instance()->init();
    }
    ...
    public function check_api_key_authentication(WP_REST_Request $request)
    {
        ...
        if (!get_option('threadnest_api_enabled', false)) {
            return false;
        }

        // Check for API key in header or query parameter
        $api_key = $request->get_header('X-Threadnest-API-Key')
    ...
    public function check_user_permissions(WP_REST_Request $request)
    {
        ...
        if (!is_user_logged_in()) {
            return new WP_Error(
                'rest_not_logged_in',
                __('You are not currently logged in.', 'threadnest'),
                array('status' => 401)
            );
        }
        ...
        if (!user_can($current_user, 'read')) {
            return new WP_Error(
                ...
                __('Sorry, you are not allowed to access the chat.', 'threadnest'),
                array('status' => 403)
            );
        }

        // Additional permission filters
        $has_permission = apply_filters('threadnest_user_can_access_chat', true, $current_user, $request);

        if (!$has_permission) {
            return new WP_Error(
                'rest_chat_access_denied',
                __('Access to chat is denied.', 'threadnest'),
                array('status' => 403)
            );
        }
    ...
    public function check_thread_access_permissions(WP_REST_Request $request)
    {
        ...
        if (!$this->user_can_access_thread($current_user_id, $thread_id)) {
            return new WP_Error(
                ...
                __('Sorry, you are not allowed to access this thread.', 'threadnest'),
                array('status' => 403)
            );
        }
    ...
    private function user_can_access_thread($user_id, $thread_id)
    {
        ...
        $thread_table = $wpdb->prefix . 'threadnest_message_threads';
        $participants_table = $wpdb->prefix . 'threadnest_message_participants';
    ...
    public function validate_file_upload($param, $request, $key)
    {
        ...
        $max_size = apply_filters('threadnest_max_file_size', 10 * 1024 * 1024);
        if ($file['size'] > $max_size) {
            return false;
        }

        // Check allowed file types
        $allowed_types = apply_filters('threadnest_allowed_file_types', array(
            'jpg',
            'jpeg',
            'png',
            'gif',
    ...
    public function get_threads(WP_REST_Request $request)
    {
        ...
        $threads_table = esc_sql($wpdb->prefix . 'threadnest_message_threads');
        $messages_table = esc_sql($wpdb->prefix . 'threadnest_messages');
        $participants_table = esc_sql($wpdb->prefix . 'threadnest_message_participants');
        $read_receipts_table = esc_sql($wpdb->prefix . 'threadnest_message_read_receipts');
    ...
    public function create_thread(WP_REST_Request $request)
    {
        ...
        if (empty($data['participants']) || !is_array($data['participants'])) {
            return new WP_Error(
                ...
                __('Participants must be provided as an array of user IDs.', 'threadnest'),
                array('status' => 400)
            );
        }
        ...
        foreach ($data['participants'] as $participant_id) {
            ...
            if (!$participant || !user_can($participant_id, 'read')) {
                return new WP_Error(
                    ...
                    sprintf(__('Invalid participant ID: %d', 'threadnest'), $participant_id),
                    array('status' => 400)
                );
            }
        }
        ...
        if ($data['type'] === 'private') {
            ...
            if (count($data['participants']) < 2) {
                return new WP_Error(
                    ...
                    __('Private chats must have exactly one other participant.', 'threadnest'),
                    array('status' => 400)
                );
            }
            ...
            if ($existing_thread) {
                return new WP_REST_Response(array(
                    ...
                    'message' => __('Thread already exists.', 'threadnest')
                ), 200);
            }
        } else {
            ...
            if (count($data['participants']) < 1) {
                return new WP_Error(
                    'invalid_participants_count',
                    __('Group chats must have at least one participant.', 'threadnest'),
                    array('status' => 400)
                );
            }
        }
        ...
        if ($result === false) {
            return new WP_Error(
                ...
                __('Failed to create thread.', 'threadnest'),
                array('status' => 500)
            );
        }
        ...
        foreach ($data['participants'] as $participant_id) {
            ...
            if ($result === false) {
                ...
                return new WP_Error(
                    ...
                    /* translators: %d: participant ID */
                    sprintf(__('Failed to add participant: %d', 'threadnest'), $participant_id),
                    array('status' => 500)
                );
            }
        }
        ...
        return new WP_REST_Response(array(
            'thread' => $formatted_thread,
            'message' => __('Thread created successfully.', 'threadnest')
        ), 201);
    }
    ...
    public function add_thread_participant(WP_REST_Request $request)
    {
        ...
        if (!in_array($role, ['member', 'admin'])) {
            return new WP_Error(
                'invalid_role',
                __('Invalid participant role.', 'threadnest'),
                array('status' => 400)
            );
        }
        ...
        if ($existing) {
            return new WP_Error(
                'already_participant',
                __('User is already a participant in this thread.', 'threadnest'),
                array('status' => 400)
            );
        }
        ...
        if ($result === false) {
            return new WP_Error(
                ...
                __('Failed to add participant.', 'threadnest'),
                array('status' => 500)
            );
        }

        return new WP_REST_Response(array(
            'message' => __('Participant added successfully.', 'threadnest')
        ), 201);
    }
    ...
    public function update_participant_role(WP_REST_Request $request)
    {
        ...
        if (!$participant) {
            return new WP_Error(
                ...
                __('Participant not found.', 'threadnest'),
                array('status' => 404)
            );
        }
        ...
        if ($participant->role === 'owner') {
            return new WP_Error(
                'cannot_modify_owner',
                __('Cannot modify the owner role.', 'threadnest'),
                array('status' => 400)
            );
        }
        ...
        if ($result === false) {
            return new WP_Error(
                'update_role_failed',
                __('Failed to update participant role.', 'threadnest'),
                array('status' => 500)
            );
        }

        return new WP_REST_Response(array(
            'message' => __('Participant role updated successfully.', 'threadnest')
        ), 200);
    }
    ...
    public function remove_thread_participant(WP_REST_Request $request)
    {
        ...
        if ($participant->role === 'owner') {
            return new WP_Error(
                'cannot_remove_owner',
                __('Cannot remove the thread owner.', 'threadnest'),
                array('status' => 400)
            );
        }
        ...
        if ($result === false) {
            return new WP_Error(
                ...
                __('Failed to remove participant.', 'threadnest'),
                array('status' => 500)
            );
        }

        return new WP_REST_Response(array(
            'message' => __('Participant removed successfully.', 'threadnest')
        ), 200);
    }
    ...
    public function get_thread_messages(WP_REST_Request $request)
    {
        ...
        $messages_table = esc_sql($wpdb->prefix . 'threadnest_messages');
        $attachments_table = esc_sql($wpdb->prefix . 'threadnest_message_attachments');
        $read_receipts_table = esc_sql($wpdb->prefix . 'threadnest_message_read_receipts');
        $users_table = esc_sql($wpdb->users);
    ...
    public function send_message(WP_REST_Request $request)
    {
        ...
        if ($result === false) {
            return new WP_Error(
                'message_send_failed',
                __('Failed to send message.', 'threadnest'),
                array('status' => 500)
            );
        }
        ...
        // Add attachments if any
        if (!empty($attachment_ids)) {
            $attachments_table = $wpdb->prefix . 'threadnest_message_attachments';
        ...
        $participants_table = $wpdb->prefix . 'threadnest_message_participants';
        $read_receipts_table = $wpdb->prefix . 'threadnest_message_read_receipts';
        ...
        do_action('threadnest_message_sent', $message, $thread_id);
    ...
    public function upload_attachment(WP_REST_Request $request)
    {
        ...
        if (empty($files['file'])) {
            return new WP_Error(
                'no_file',
                __('No file was uploaded.', 'threadnest'),
                array('status' => 400)
            );
        }
        ...
        $upload_overrides = array(
            'test_form' => false,
            'mimes' => apply_filters('threadnest_upload_mimes', array(
        ...
        if ($movefile && !isset($movefile['error'])) {
            ...
            if ($result === false) {
                ...
                return new WP_Error(
                    ...
                    __('Failed to save attachment information.', 'threadnest'),
                    array('status' => 500)
                );
            }
            ...
            return new WP_Error(
                'upload_failed',
                isset($movefile['error']) ? $movefile['error'] : __('File upload failed.', 'threadnest'),
                array('status' => 500)
            );
        }
    }
    ...
    public function handle_typing_indicator(WP_REST_Request $request)
    {
        ...
        $transient_key = "threadnest_typing_{$thread_id}_{$user_id}";
        ...
        do_action('threadnest_typing_indicator', $thread_id, $user_id, $is_typing);
    ...
    public function mark_messages_read(WP_REST_Request $request)
    {
        ...
        $messages_table = $wpdb->prefix . 'threadnest_messages';
        $read_receipts_table = $wpdb->prefix . 'threadnest_message_read_receipts';
    ...
    private function send_socket_webhook($endpoint, $data)
    {
        ...
        $socket_url = get_option('threadnest_socket_server_url', 'http://localhost:3001');
    ...
    public function get_users(WP_REST_Request $request)
    {
        ...
        if (!$current_user || !$current_user->ID) {
            return new WP_Error(
                'rest_forbidden',
                __('You must be logged in to access this endpoint.', 'threadnest'),
                array('status' => 401)
            );
        }
        ...
        $args = apply_filters('threadnest_user_query_args', $args);
        ...
        foreach ($users as $user) {
            ...
            $user_data = apply_filters('threadnest_user_response_data', $user_data, $user);
    ...
    private function find_private_thread($user1_id, $user2_id)
    {
        ...
        $threads_table = $wpdb->prefix . 'threadnest_message_threads';
        $participants_table = $wpdb->prefix . 'threadnest_message_participants';
    ...
    public function get_typing_status($thread_id)
    {
        ...
        $transient_prefix = "threadnest_typing_{$thread_id}_";
    ...
    private function validate_message_content($content, $content_type)
    {
        if (empty(trim($content))) {
            return new WP_Error(
                'empty_content',
                __('Message content cannot be empty.', 'threadnest'),
                array('status' => 400)
            );
        }

        // Content length validation
        $max_length = apply_filters('threadnest_max_message_length', 10000);
        if (strlen($content) > $max_length) {
            return new WP_Error(
                'content_too_long',
                /* translators: %d: maximum number of characters */
                sprintf(__('Message content cannot exceed %d characters.', 'threadnest'), $max_length),
                array('status' => 400)
            );
        }
        ...
        if ($content_type === 'reaction') {
            ...
            if (!preg_match('/^[\p{So}\p{Sk}]{1,5}$/u', $content)) {
                return new WP_Error(
                    'invalid_reaction',
                    __('Invalid reaction format.', 'threadnest'),
                    array('status' => 400)
                );
            }
        }
    ...
    {
        $limit_key = "threadnest_rate_limit_{$action}_{$user_id}";
        $current_count = get_transient($limit_key);

        $limits = apply_filters('threadnest_rate_limits', array(
        ...
        if ($current_count >= $limit['count']) {
            return new WP_Error(
                ...
                sprintf(
                    ...
                    __('Rate limit exceeded. Maximum %1$d %2$s per %3$d seconds.', 'threadnest'),
                    $limit['count'],
                    $action,
                    $limit['period']
                ),
    ...
    public function get_message_read_receipts(WP_REST_Request $request)
    {
        ...
        if (!$message) {
            return new WP_Error(
                'message_not_found',
                __('Message not found.', 'threadnest'),
                array('status' => 404)
            );
        }
    ...
    public function get_admin_settings(WP_REST_Request $request)
    {
        if (!current_user_can('manage_options')) {
            return new WP_Error('rest_forbidden', __('You do not have permission to access settings.', 'threadnest'), array('status' => 403));
        }

        return new WP_REST_Response(array(
            'socketUrl' => get_option('threadnest_socket_server_url', 'http://localhost:3001'),
            'typingIndicators' => get_option('threadnest_enable_typing_indicators', '1') === '1',
            'readReceipts' => get_option('threadnest_enable_read_receipts', '1') === '1',
            'presenceStatus' => get_option('threadnest_enable_presence_status', '1') === '1',
            'apiKey' => get_option('threadnest_api_key', ''),
            'apiEnabled' => get_option('threadnest_api_enabled', false),
            'apiUser' => get_option('threadnest_api_user_id', '1')
        ), 200);
    }
    ...
    {
        if (!current_user_can('manage_options')) {
            return new WP_Error('rest_forbidden', __('You do not have permission to save settings.', 'threadnest'), array('status' => 403));
        }

        $data = $request->get_json_params();

        update_option('threadnest_socket_server_url', sanitize_url($data['socketUrl']));
        update_option('threadnest_enable_typing_indicators', $data['typingIndicators'] ? '1' : '0');
        update_option('threadnest_enable_read_receipts', $data['readReceipts'] ? '1' : '0');
        update_option('threadnest_enable_presence_status', $data['presenceStatus'] ? '1' : '0');
        update_option('threadnest_api_enabled', $data['apiEnabled']);
        update_option('threadnest_api_user_id', absint($data['apiUser']));

        if (!empty($data['apiKey'])) {
            update_option('threadnest_api_key', sanitize_text_field($data['apiKey']));
        }

        return new WP_REST_Response(array(
            'success' => true,
            'message' => __('Settings saved successfully.', 'threadnest')
        ), 200);
    }
    ...
    {
        if (!current_user_can('manage_options')) {
            return new WP_Error('rest_forbidden', __('You do not have permission to generate API keys.', 'threadnest'), array('status' => 403));
        }

        $api_key = 'threadnest_api_key_' . wp_generate_password(40, false, false);
        update_option('threadnest_api_key', $api_key);
    ...
    {
        if (!current_user_can('manage_options')) {
            return new WP_Error('rest_forbidden', __('You do not have permission to view migrations.', 'threadnest'), array('status' => 403));
        }

        global $wpdb;
        $tables = array(
            'wp_threadnest_message_threads' => 'create_message_threads_table',
            'wp_threadnest_messages' => 'create_messages_table',
            'wp_threadnest_message_attachments' => 'create_attachments_table',
            'wp_threadnest_message_read_receipts' => 'create_read_receipts_table',
            'wp_threadnest_message_participants' => 'create_message_participants_table'
        );
    ...
    {
        if (!current_user_can('manage_options')) {
            return new WP_Error('rest_forbidden', __('You do not have permission to run migrations.', 'threadnest'), array('status' => 403));
        }
        ...
        return new WP_REST_Response(array(
            'success' => true,
            'message' => __('Migrations completed successfully.', 'threadnest')
        ), 200);
    }
    ...
    {
        if (!current_user_can('manage_options')) {
            return new WP_Error('rest_forbidden', __('You do not have permission to rollback migrations.', 'threadnest'), array('status' => 403));
        }

        // Trigger rollback
        ThreadnestDatabaseManager::instance()->rollback_migrations();
        do_action('threadnest_rollback_migrations');

        return new WP_REST_Response(array(
            'success' => true,
            'message' => __('Migrations rolled back successfully.', 'threadnest')
        ), 200);
    }
}
