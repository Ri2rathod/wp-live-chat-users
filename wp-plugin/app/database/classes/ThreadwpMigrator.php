...
use Threadwp\database\CLI\ThreadwpMigrate;
use WP_Error;

defined('ABSPATH') or die('Something went wrong');

class ThreadwpMigrator {
    ...
    protected $table_name = 'threadwp_migrations';

    /**
     * @param string $command_name
     *
     * @return ThreadwpMigrator Instance
     */
    public static function instance( $command_name = 'threadwp') {
        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof ThreadwpMigrator ) ) {
            self::$instance = new ThreadwpMigrator();
    ...
    public function init( $command_name ) {
        ...
        if ( defined( 'WP_CLI' ) && WP_CLI ) {
            \WP_CLI::add_command( $command_name . ' migrate', ThreadwpMigrate::class );
        }
    }
    ...
    protected function get_all_migration_paths() {
        ...
        $migration_paths = apply_filters( 'threadwp_wp_migrations_paths_detailed', $migration_paths );

        // Also support the old filter format for backward compatibility
        $old_format_paths = apply_filters( 'threadwp_wp_migrations_paths', array( $this->get_migrations_path() ) );
    ...
    protected function get_migrations_path() {
        return apply_filters( 'threadwp_wp_migrations_path', THREADWP_DIR . '/app/database/migrations' );
    }
        ...
        foreach ( $migrations as $file => $migration_info ) {
            ...
            if ( false === $fq_class_name ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadwpMigrator] Migration class not found: {$class_name} in file {$file} (Plugin: {$plugin})" );
                }
                continue;
            }
            ...
            if ( ! method_exists( $migration_instance, $method ) ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadwpMigrator] Method '{$method}' not found in migration class: {$class_name} ({$file}) (Plugin: {$plugin})" );
                }
                continue;
            }
            ...
            try {
                ...
                if ( $rollback ) {
                    ...
                    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                        error_log( "[ThreadwpMigrator] Successfully rolled back migration: {$name} (Plugin: {$plugin})" );
                    }
                } else {
                    ...
                    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                        error_log( "[ThreadwpMigrator] Successfully ran migration: {$name} (Plugin: {$plugin})" );
                    }
                }
            } catch ( \Exception $e ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadwpMigrator] Error running migration {$name} (Plugin: {$plugin}): " . $e->getMessage() );
                }
     ...
     * @param string $plugin_key Plugin key to determine which path to use (optional, defaults to 'threadwp')
     *
     * @return string|WP_Error Name of created migration file on success, WP_Error
     *                         instance on failure.
     */
    public function scaffold( $migration_name, $plugin_key = 'threadwp' ): string|WP_Error {
        // Get migration paths
        $migration_paths = $this->get_all_migration_paths();
        ...
        if ( ! is_dir( $target_path ) ) {
            ...
            if ( ! $wp_filesystem->mkdir( $target_path, 0755 ) ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadwpMigrator] Unable to create migrations folder {$target_path}" );
                }
                return new \WP_Error(
        ...
        $stub_dir  = THREADWP_DIR . '/app/database/stubs';
        $stub_path = apply_filters( 'threadwp_migration_stub_path', "{$stub_dir}/migration.stub" );
        $stub      = file_get_contents( $stub_path );

        if ( ! $stub ) {
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( "[ThreadwpMigrator] Unable to read migration stub file: {$stub_path}" );
            }
            return new \WP_Error(
                'stub_file_error',
        ...
        if ( ! file_put_contents( $file_path, $boilerplate ) ) {
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( "[ThreadwpMigrator] Unable to create migration file: {$file_path}" );
            }
            return new \WP_Error(
                'file_creation_error',
        ...
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            error_log( "[ThreadwpMigrator] Successfully created migration file: {$file_path} (Plugin: {$plugin_key})" );
        }
        return $filename;
    }
