...
use Threadnest\database\CLI\ThreadnestMigrate;
use WP_Error;

defined('ABSPATH') or die('Something went wrong');

class ThreadnestMigrator {
    ...
    protected $table_name = 'threadnest_migrations';

    /**
     * @param string $command_name
     *
     * @return ThreadnestMigrator Instance
     */
    public static function instance( $command_name = 'threadnest') {
        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof ThreadnestMigrator ) ) {
            self::$instance = new ThreadnestMigrator();
    ...
    public function init( $command_name ) {
        ...
        if ( defined( 'WP_CLI' ) && WP_CLI ) {
            \WP_CLI::add_command( $command_name . ' migrate', ThreadnestMigrate::class );
        }
    }
    ...
    protected function get_all_migration_paths() {
        ...
        $migration_paths = apply_filters( 'threadnest_wp_migrations_paths_detailed', $migration_paths );

        // Also support the old filter format for backward compatibility
        $old_format_paths = apply_filters( 'threadnest_wp_migrations_paths', array( $this->get_migrations_path() ) );
    ...
    protected function get_migrations_path() {
        return apply_filters( 'threadnest_wp_migrations_path', THREADNEST_DIR . '/app/database/migrations' );
    }
        ...
        foreach ( $migrations as $file => $migration_info ) {
            ...
            if ( false === $fq_class_name ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadnestMigrator] Migration class not found: {$class_name} in file {$file} (Plugin: {$plugin})" );
                }
                continue;
            }
            ...
            if ( ! method_exists( $migration_instance, $method ) ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadnestMigrator] Method '{$method}' not found in migration class: {$class_name} ({$file}) (Plugin: {$plugin})" );
                }
                continue;
            }
            ...
            try {
                ...
                if ( $rollback ) {
                    ...
                    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                        error_log( "[ThreadnestMigrator] Successfully rolled back migration: {$name} (Plugin: {$plugin})" );
                    }
                } else {
                    ...
                    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                        error_log( "[ThreadnestMigrator] Successfully ran migration: {$name} (Plugin: {$plugin})" );
                    }
                }
            } catch ( \Exception $e ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadnestMigrator] Error running migration {$name} (Plugin: {$plugin}): " . $e->getMessage() );
                }
     ...
     * @param string $plugin_key Plugin key to determine which path to use (optional, defaults to 'threadnest')
     *
     * @return string|WP_Error Name of created migration file on success, WP_Error
     *                         instance on failure.
     */
    public function scaffold( $migration_name, $plugin_key = 'threadnest' ): string|WP_Error {
        // Get migration paths
        $migration_paths = $this->get_all_migration_paths();
        ...
        if ( ! is_dir( $target_path ) ) {
            ...
            if ( ! $wp_filesystem->mkdir( $target_path, 0755 ) ) {
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "[ThreadnestMigrator] Unable to create migrations folder {$target_path}" );
                }
                return new \WP_Error(
        ...
        $stub_dir  = THREADNEST_DIR . '/app/database/stubs';
        $stub_path = apply_filters( 'threadnest_migration_stub_path', "{$stub_dir}/migration.stub" );
        $stub      = file_get_contents( $stub_path );

        if ( ! $stub ) {
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( "[ThreadnestMigrator] Unable to read migration stub file: {$stub_path}" );
            }
            return new \WP_Error(
                'stub_file_error',
        ...
        if ( ! file_put_contents( $file_path, $boilerplate ) ) {
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( "[ThreadnestMigrator] Unable to create migration file: {$file_path}" );
            }
            return new \WP_Error(
                'file_creation_error',
        ...
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            error_log( "[ThreadnestMigrator] Successfully created migration file: {$file_path} (Plugin: {$plugin_key})" );
        }
        return $filename;
    }
